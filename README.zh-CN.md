...
- 翻译图片中的可见文字。
- 对阿拉伯语等 RTL 语言默认启用严格 RTL-aware 版式适配；只有当 QA 判断真正的 RTL 版式更差时，才回退到仅翻译文案的保守模式。
...
每次任务都应该放进独立 run 文件夹。保持工作区根目录干净，不要把源图、模型 raw 输出、临时脚本、QA 文件、zip 和最终交付文件混在同一层。
...
- `final/`：只放可直接上传投放的文件。
- `qa/`：放 manifest、QA 总览图、QA notes 和日志。
- `work/`：放模型 raw 输出、实验图、临时脚本和确定性裁切中间文件。
- `source/`：复制一份原始输入图，方便回溯。
- `Flagged by Culture-Aware QA/`：放需要投放前人工复核的文件。
...
对于阿拉伯语等 RTL 语言，skill 会优先尝试真正符合 RTL 阅读习惯的版式适配；如果 QA 判断适配后效果更差，则回退到仅翻译文案的版本。

## RTL 感知本地化

对于阿拉伯语等从右到左阅读的语言，这个 skill 采用质量优先的回退策略：默认先生成真正的 RTL-aware 版式；如果适配后的版式不自然、阅读效率更差或影响品牌识别，则回退到仅翻译文案的保守模式。

![RTL 感知本地化 QA 回退 demo](./examples/rtl-aware-localization.zh-CN.png)

- **方案 A：开启 RTL-aware（RTL 语言默认）**：主文案默认右对齐，让信息流从右侧开始，适配 CTA 位置/方向，并在需要时调整局部 UI、对话气泡、产品或主体关系，让画面自然按从右到左阅读。
- **方案 B：关闭 RTL-aware / 回退模式**：只翻译可见文案，尽量保留原始构图和整体布局。

建议阿拉伯语等 RTL 语言默认使用方案 A；只有当真正的 RTL-aware 版式导致构图失衡、品牌识别受损或文字排版变差时，使用方案 B。这是一个默认启用、但受 QA 控制的策略，不是强制对所有阿语素材进行整图镜像，也不会默认翻转 logo 或大幅移动品牌识别。

RTL-aware QA 应该拒绝这些结果：阿语/RTL 主标题、正文或 CTA 无明确理由仍左对齐或只是居中；视觉阅读路径仍明显从左侧开始；Arabic + Latin 品牌/产品混排顺序或间距别扭。如果最终版本保留了明显 LTR 构图，需要在 QA notes 里说明原因，并标明是否为 copy-only fallback。
...
阿拉伯语默认先生成真正的 RTL-aware 版式：阿语主文案右对齐、右侧信息优先、CTA 符合 RTL 阅读流。如果 QA 判断真正的 RTL 版式更差，则回退到仅翻译文案、保留原整体布局的版本。
...
对于阿拉伯语等 RTL 语言，skill 会优先尝试真正符合 RTL 阅读习惯的版式适配，包括主文案右对齐和从右到左的信息流；如果 QA 判断适配后效果更差，则回退到仅翻译文案的版本。
...
运行产物默认放在 `ad-localization-runs/` 下，并已被 Git 忽略。
